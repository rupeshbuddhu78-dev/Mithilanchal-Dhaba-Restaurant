...
export async function ensureRestaurantSeed() {
  ...
  for (const seedUser of seedUsers) { const [found] = await db.select().from(users).where(eq(users.openId, seedUser.openId)).limit(1); if (!found) await db.insert(users).values({ ...seedUser, email: null, loginMethod: "seed", stripeCustomerId: null, lastSignedIn: new Date(0) }); }
