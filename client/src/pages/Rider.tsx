...
export default function Rider() {
  ...
  const riderLogin = trpc.auth.passwordLogin.useMutation({
    onSuccess: () => utils.auth.me.invalidate(),
    onError: error => toast.error(error.message),
  });
  const recovery = trpc.auth.passwordResetRequest.useMutation({
    onSuccess: () => {
      setRecoveryOpen(false);
      toast.success("If an eligible account exists, restaurant staff will follow up.");
    },
    onError: () => toast.success("If an eligible account exists, restaurant staff will follow up."),
  });
  const tasks = trpc.operations.rider.tasks.useQuery(undefined, { enabled: allowed });
  const update = trpc.operations.rider.updateStatus.useMutation({
    onSuccess: () => {
      utils.operations.rider.tasks.invalidate();
  ...
  const stopTracking = trpc.operations.rider.stopTracking.useMutation({
    onSuccess: () => toast.success("Live location sharing stopped"),
    onError: error => toast.error(error.message),
  });
  ...
  useEffect(() => {
    ...
    const watchId = navigator.geolocation.watchPosition(
      position => {
        const now = Date.now();
        if (now - lastLocationSentAt.current < 12_000) return;
        lastLocationSentAt.current = now;
        const location = { latitude: position.coords.latitude, longitude: position.coords.longitude };
        setCurrentLocation(location);
        tracking.mutate({ orderId: trackingOrderId, ...location, locationConsent: true });
      },
      () => {
        setTrackingOrderId(null);
        toast.error("Live location permission was not granted. Address directions remain available.");
      },
  ...
  const beginTracking = (orderId: number, destination: DeliveryDestination, routeWindow?: Window | null) => {
    if (!navigator.geolocation) {
      routeWindow?.close();
      return toast.error("Location is not available in this browser. Address directions remain available.");
    }
    navigator.geolocation.getCurrentPosition(
      position => {
        ...
        tracking.mutate({ orderId, ...location, locationConsent: true });
        const route = buildLiveDirectionsUrl(location, destination);
        if (route && routeWindow) routeWindow.location.replace(route);
        else routeWindow?.close();
        toast.success(route ? "Live location sharing started and directions opened." : "Live location sharing started for this delivery.");
      },
      () => {
        routeWindow?.close();
        toast.error("Location permission was not granted. Address directions remain available.");
      },
      { enableHighAccuracy: true, timeout: 15_000 },
    );
  };

  const startDelivery = (orderId: number, destination: DeliveryDestination) => {
    const routeWindow = openRouteWindow();
    update.mutate(
      { orderId, status: "out_for_delivery" },
      {
        onSuccess: () => beginTracking(orderId, destination, routeWindow),
        onError: () => routeWindow?.close(),
      },
    );
  };

  const startLiveSharing = (orderId: number, destination: DeliveryDestination) => beginTracking(orderId, destination, openRouteWindow());
  const endTracking = (orderId: number) => {
    setTrackingOrderId(null);
    stopTracking.mutate({ orderId });
  };
  ...
  const openLiveDirections = (destination: DeliveryDestination) => {
    const navigate = (coordinates: Coordinates, routeWindow?: Window | null) => {
      const route = buildLiveDirectionsUrl(coordinates, destination);
