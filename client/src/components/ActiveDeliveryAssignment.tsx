  ...
  return <section className="mt-7 rounded-3xl bg-white p-6">
    ...
    <div className="mt-5 space-y-3">
      {assignments.data?.map(({ assignment, order, rider }) => {
        const selected = riderByOrder[order.id] ?? String(rider.id);
        return <div key={assignment.id} className="flex flex-wrap items-center gap-3 rounded-2xl border border-[#272119]/10 p-4">
          <div className="min-w-44 flex-1"><p className="font-bold">{order.orderNo}</p><p className="mt-1 text-xs text-[#74695b]">Current rider: {rider.name || rider.email || `Rider ${rider.id}`}</p></div>
          ...
          <Button disabled={reassign.isPending || selected === String(rider.id)} onClick={() => reassign.mutate({ orderId: order.id, riderUserId: Number(selected) })} className="rounded-xl bg-[#ae3f25]">Update rider</Button>
        </div>;
      })}
    </div>
  </section>;
